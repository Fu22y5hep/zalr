...
class Command(BaseCommand):
    help = 'Stage 7: Generate long summaries for judgments for all courts or specific court for a given year'
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument('--batch-size', type=int, default=5, help='Number of judgments to process in one batch (default: 5)')
        parser.add_argument('--judgment-id', type=str, help='Process only this specific judgment ID (optional)')
        parser.add_argument('--force', action='store_true', help='Force regeneration of long summaries for judgments that already have them')
    ...
    def setup_logging(self):
        ...
        log_filename = os.path.join(log_dir, f'stage7_long_summary_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log')
        logging.basicConfig(
            level=logging.INFO,
    ...
    def generate_long_summaries_for_court_year(self, court, year, batch_size=5, force=False, min_reportability=0.0):
        """
        Generate long summaries for judgments from the specified court and year.
        ...
        try:
            ...
            # If not force, only process judgments without long summaries
            if not force:
                query = query.filter(long_summary__isnull=True)
            
            judgment_count = query.count()
            
            if judgment_count == 0:
                self.stdout.write(self.style.WARNING(f"[{court}] No judgments found that need long summaries for {court} {year}"))
                return 0
            
            self.stdout.write(self.style.SUCCESS(f"[{court}] Found {judgment_count} judgments that need long summaries"))
            
            # Process judgments for long summaries
            processed_count = 0
            ...
            for i in range(0, len(judgment_ids), batch_size):
                ...
                try:
                    # Process this batch
                    batch_result = summarize_judgments(judgment_ids=batch, force=force)
                    if batch_result:
                        processed_count += len(batch_result)
                        self.stdout.write(self.style.SUCCESS(f"[{court}] Successfully processed batch of {len(batch_result)} judgments"))
                    else:
            ...
            self.stdout.write(self.style.ERROR(f"[{court}] Error generating long summaries: {str(e)}"))
            return 0
    ...
    def handle(self, *args, **options):
        ...
        batch_size = options.get('batch-size', 5)
        ...
        if judgment_id:
            self.stdout.write(self.style.SUCCESS(f"Processing long summary for single judgment: {judgment_id}"))
            try:
                # Process just this judgment
                result = summarize_judgments(judgment_ids=[judgment_id], force=force)
                if result:
                    self.stdout.write(self.style.SUCCESS(f"Successfully generated long summary for judgment {judgment_id}"))
                else:
                    self.stdout.write(self.style.WARNING(f"No long summary generated for judgment {judgment_id}"))
                return
            except Exception as e:
        ...
        for court in courts_to_process:
            self.stdout.write(self.style.SUCCESS(f"[{court}] STAGE 7: Generating long summaries for {court} {year}"))
            
            try:
                processed_count = self.generate_long_summaries_for_court_year(court, year, batch_size, force, min_reportability)
                if processed_count > 0:
                    self.stdout.write(self.style.SUCCESS(f"[{court}] Successfully generated long summaries for {processed_count} judgments"))
                    logger.info(f"[{court}] Generated long summaries for {processed_count} judgments")
                    success_count += 1
                    total_processed += processed_count
                else:
                    self.stdout.write(self.style.WARNING(f"[{court}] No judgments needed long summaries for {court} {year}"))
                    logger.info(f"[{court}] No judgments needed long summaries")
            except Exception as e:
                self.stdout.write(self.style.ERROR(f"[{court}] Error generating long summaries: {str(e)}"))
        ...
        # Final summary
        self.stdout.write(self.style.SUCCESS(f"Stage 7 complete: Generated long summaries for {total_processed} judgments across {success_count} courts, failed {failure_count} courts")) 